  ...
  tabsetPanel(id="outerTabs",
    ...
    tabPanel("Grading",
      radioButtons("currentProblem", "Problem to grade", choices=staticCurrentProblem,
                   inline=TRUE),
      fluidRow(column(1, actionButton("priorStudent", "Prior")),
               column(5, selectInput("selectStudent",
                                     label=paste(length(staticSelectStudent) - 1,
                                                 "Students (Canvas name; email)"),
                                     staticSelectStudent,
                                     width="100%")),
               column(1, actionButton("nextStudent", "Next")),
               column(2, actionButton("runAllStudents", "Run all students"))),
      verbatimTextOutput("filesForOne"),
